	...
	<script>
				...
				for(board of list){
					...
					`<div class="title-inner">
                            ...
                            <div class="title">
								...
								/snsboard/download/`+board.fileLoca+`/`+board.fileName+`/`+`">이미지 다운로드</a>
                            </div>
                        </div>
                        <div class="content-inner">
                            <!--내용영역-->
                        ...
                        <div class="image-inner">
                            <!-- 이미지영역 -->
                            <a href="` + board.bno + `">
                                <img data-bno="`+board.bno+`" src="${pageContext.request.contextPath}
								/snsboard/display/`+board.fileLoca+`/`+board.fileName+`">
                            </a>
                        </div>
		...
		$contentDiv.addEventListener('click', e =>{
			...
			.then(data => {
				...
				document.getElementById('snsImg').setAttribute('src',
												'${pageContext.request.contextPath}/snsboard/display/'+data.fileLoca+'/'+data.fileName+''); //display 요청
			});
        ...
        */
		...
		$contentDiv.addEventListener('click', e => {
			...
			.then(result => {
				...
				if(result === 'like'){
					...
					const $cnt = e.target.parentNode.previousElementSibling.children[1]; //유사배열idx 0부터
		...
		function parseTime(regDate){
                ...
                let time;
                if(gap < 60 * 60 * 24 *1000){//밀리초니까 1000 곱해준다. 시간차가 하루보다 작으면
                    if(gap < 60 * 60 * 1000){//시간차가 1시간보다 작으면
                        time = '방금 전';
                    }else{
                        time = parseInt(gap / (1000*60*60)) + '시간 전';
                    }
                }else if(gap < 60 * 60 * 24 * 30 * 1000){//한달보다 작으면
