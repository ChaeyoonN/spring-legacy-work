...
public class ReplyController {
	...
	@PostMapping()
//	@ResponseBody
	public String replyRegist(@RequestBody ReplyRequestDTO dto) {//@RequestBody는 json->dto 알아서 변환해줌
	...
	public Map<String, Object> getList(@PathVariable int bno, @PathVariable int pageNum) {
		...
		/*
		 ...
		 2. Mapper 인터페이스에서 복수의 값을 전달하기 위해 Map을 쓸지, @Param을 쓸 지 결정.
		 3. ReplyMapper.xml에 sql문을 페이징 쿼리로 작성.
		 4. 클라이언트 측으로 DB에서 조회한 댓글 목록을 보낼 때,
		 페이징을 위한 댓글의 총 개수도 함께 보내줘야 합니다.
		 근데 우리는 return을 한 개만 쓸 수 있으니까, 복수 개의 값을 리턴하기 위해
		 리턴 타입을 Map으로 줄 지, 객체를 디자인해서 줄 지를 결정합니다.
		 (댓글 목록 리스트과 전체 댓글 개수를 함께 전달할 예정.) -> 일회성으로 쓸 거니까 Map으로 클라이언트에게 전달.
		 */
	...
	//댓글 삭제 요청
	@DeleteMapping("/{rno}")
	public String delete(@PathVariable int rno, @RequestBody String replyPw) {//body:replyPw
//		System.out.println("댓글 삭제 요청 들어옴: " + replyPw);
