...
public class ScoreSpringRepository implements IScoreRepository {
	...
	//이 필드 타입을 매개값으로 받는 생성자 자동 주입: this.필드 = 필드타입 매개변수
	...
	public List<Score> findAll() {
		...
		//sql 실행 후 그 결과를 ScoreMapper대로 포장해서 리스트에 넣어 반환
		//template이 생성된 객체의 메서드 호출함
	}
	...
	public Score findByStuNum(int stuNum) {
		...
		try {
			...
			//sql 실행 후 그 결과를 ScoreMapper대로 포장해서 객체 반환
			
		} catch (Exception e) {
			// 저회 결과가 없다면 catch문이 실행
			return null;
		}
